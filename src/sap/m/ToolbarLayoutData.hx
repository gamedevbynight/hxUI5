...
<b>Note:</b> The {@link sap.m.ToolbarSpacer} is a flex control that is intended to control its own behavior, thus <code>sap.m.ToolbarLayoutData</code> is not supported as value for the <code>layoutData</code> aggregation of {@link sap.m.ToolbarSpacer} and if set it's ignored.
*/
