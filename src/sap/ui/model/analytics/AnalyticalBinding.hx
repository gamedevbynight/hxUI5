...
Note on the handling of different count modes: The AnalyticalBinding always uses the OData $inlinecount system query option to determine the total count of matching entities. It ignores the default count mode set in the ODataModel instance and the count mode specified in the binding parameters. If the default count mode is <code>None</code>, a warning is added to the log to remind the application that OData requests generated by the AnalyticalBinding will include a $inlinecount. If a count mode has been specified in the binding parameters, an error message is logged if it is <code>None</code>, because the binding still adds the $inlinecount to OData requests. If a binding count mode is set to <code>Request</code> or <code>Both</code>, a warning is logged to remind the application that the OData requests generated by the AnalyticalBinding include a $inlinecount.
*/
...
Counting takes place at the lowest aggregation level defined by the possible value combinations for the complete set of dimension properties included in the bound entity set. This means that intermediate aggregate entities with sub-totals at higher aggregation levels are not counted.
	* @return	The total number of addressed entities in the OData entity set, or -1 if the number is not yet known or if the binding parameter <code>provideTotalResultSize</code> is set to <code>false</code>
	*/
